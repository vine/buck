...
class PexInfo(object):
  ...
  native_libraries: {name: str}     # list of native libraries (dependencies of python extensions)

  # Environment options
  pex_root: ~/.pex                   # root of all pex-related files
  entry_point: string                # entry point into this pex
  zip_safe: True, default False      # is this pex zip safe? (forced to False in the 
  ...
  def zip_safe(self):
    ...
    if self._native_libraries:
      self.debug('native libraries added, forcing zip_safe to False')
      return False
  ...
  def add_native_library(self, filename, sha):
    self._native_libraries[filename] = sha
