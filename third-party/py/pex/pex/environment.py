...
class PEXEnvironment(Environment):
  ...
  def maybe_reexec_for_library_path(self):
    ...
    if os.path.isdir(self._pex):
      ...
      exploded_dir = os.path.join(self._pex_info.zip_unsafe_cache, self._pex_info.code_hash)
    ...
    assert not os.environ.get('_PEX_ADDED_LIBS'), 'Expected pex libs at %s: %s' % (expected_path, paths)
    paths.insert(0, expected_path)
    os.environ[var_name] = ':'.join(paths)
    os.environ['_PEX_ADDED_LIBS'] = '1'
  ...
  def _activate(self):
    ...
    pex_path = self._pex
    if not self._pex_info.zip_safe and os.path.isfile(pex_path):
      pex_path = self.force_local(self._pex, self._pex_info)
      self.update_module_paths(pex_path)
